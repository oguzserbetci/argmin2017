    ...
    for (i, (training, test)) in tqdm(enumerate(outer.split(Xe)), desc='outer'):
        ...
        for k, param in tqdm(enumerate(paramsearch), desc='params'):
            ...
            for Y_pred, Y_true, output, func in zip(Y_preds, Y_trues, outputs, f1_functions):
                ...
                weighted_cat = utils.binary_f1(Y_true, Y_pred, average='weighted')
                weighted = func(Y_true, Y_pred, average='weighted')
                singles = func(Y_true, Y_pred, average=None)
                print('macro:', macro)
                print('macro_cat:', macro_cat)
                print('weighted_cat:', weighted_cat)
                print('weighted:', weighted)
                print('categories:', singles)
                tests[param_str].setdefault('{}_macro'.format(output), []).append(macro)
                tests[param_str].setdefault('{}_macro_cat'.format(output).format(output), []).append(macro_cat)
                tests[param_str].setdefault('{}_weighted_cat'.format(output).format(output), []).append(weighted_cat)
                tests[param_str].setdefault('{}_weighted'.format(output), []).append(weighted)
                tests[param_str].setdefault('{}_cat0'.format(output), []).append(singles[0])
